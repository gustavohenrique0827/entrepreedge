...
const initializeSettings = () => {
  if (localStorage.getItem('fenix_darkMode') === 'true') {
    document.documentElement.classList.add('dark');
  } else {
    document.documentElement.classList.remove('dark');
  }
  
  const fontSize = localStorage.getItem('fenix_fontSize') || 'medium';
  document.documentElement.setAttribute('data-font-size', fontSize);
  
  const language = localStorage.getItem('language') || 'pt-BR';
  document.documentElement.lang = language.split('-')[0];
  
  const companyName = localStorage.getItem('companyName') || 'Fenix';
  document.title = `${companyName} - Plataforma Empresarial`;
  ...
  // Ensure theme colors are set from localStorage
  const primaryColor = localStorage.getItem('fenix_primaryColor') || '#ff5722';
  const secondaryColor = localStorage.getItem('fenix_secondaryColor') || '#9c27b0';
  document.documentElement.style.setProperty('--primary-color', primaryColor);
  document.documentElement.style.setProperty('--secondary-color', secondaryColor);
  
  // Set critical CSS variables that buttons and other elements depend on
...
const App = () => {
  ...
  useEffect(() => {
    // Re-apply settings when component mounts
    initializeSettings();
    
    // For hot reloads and handling storage events from other tabs
    ...
    // Apply colors on navigation (SPA navigation)
    const handleRouteChange = () => {
      if (window.fenixColors?.applyColors) {
        window.fenixColors.applyColors();
      }
    };
